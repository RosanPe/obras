...
export class MotorDeRegrasService {
  ...
  private aplicarRegras(mapa: Map<string, LinhaResultado>, ponto: PontoMedicao, multiplicadorPonto: number): void {
    for (const regra of this.contexto.regrasMaoObra) {
      ...
      const estruturasContexto = regra.gatilho.tipo === "estrutura"
  ...
  private regraAtendePonto(regra: RegraMaoObra, ponto: PontoMedicao): boolean {
    ...
    if (regra.gatilho.tipo === "estrutura") {
      ...
      return estruturaIdsPonto.includes(regra.gatilho.estruturaId);
    }

    if (regra.gatilho.tipo === "contexto") {
      return Boolean(this.estruturaIdsDoPonto(ponto).length || ponto.posteId || ponto.caboBTId || ponto.caboMTId);
    }
  ...
  private estruturaDoPonto(ponto: PontoMedicao): Estrutura | undefined {
    return this.contexto.estruturas.find((item) => item.id === ponto.estruturaId);
  }
  ...
  private estruturasDoPonto(ponto: PontoMedicao): Estrutura[] {
    return this.estruturaIdsDoPonto(ponto)
      .map((id) => this.contexto.estruturas.find((item) => item.id === id))
  ...
  private estruturasDaRegraNoPonto(regra: RegraMaoObra, ponto: PontoMedicao): Array<Estrutura | undefined> {
    ...
    if (!regra.gatilho.estruturaId) return estruturas;
    return estruturas.filter((estrutura) => estrutura.id === regra.gatilho.estruturaId);
  }
